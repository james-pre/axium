...
{#snippet task_checkbox(node: TaskTreeNode, isMirror: boolean = false)}
	{const task = $derived(node.task),
		directCompletion = $derived(node.subtasks.length ? node.subtasks.filter(s => s.task.completed).length / node.subtasks.length : 0)}
...
{#snippet task_tree(node: TaskTreeNode, depth: number = 0, isCompletedMirror: boolean = false)}
	...
	{#each node.subtasks as sub (sub.task.id)}
		...
		{/if}
	{/each}
	{const completedSubs = $derived(node.subtasks.filter(sub => sub.all == 'completed'))}
